...
contract Kharacter is ERC721("Kharacter", "KHAR") {
    ...
    function exploit(uint256 id, string[] calldata picks)
        public
        returns (bool)
    {
        bool cracked = false;
        if (lockCount[id] > 0) {
            uint256 passCount = 0;
            for (uint256 i = 0; i < lockCount[id]; i++) {
                (bool success, ) = lock[id][i].staticcall(
                    abi.encodeWithSignature("unlock(string)", picks[i])
                );
                if (success) {
